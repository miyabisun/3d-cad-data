...
| `gfb_recess(wall, floor_t, r)` | 底 1 マスぶんの窪みの cut。`gfb_base_cell` の 3 段を `wall` だけ内側・`floor_t` だけ上へ寄せた相似形 (45°)。天面の隅は R `r` |
| `gfb_card_case(cols, rows, units, wall, floor_t, card, clearance, r)` | `gfb_bin` (ラベル無し) の内側を壁の上端まで埋め、左手前の隅に `card + clearance` のポケット、右奥のマスに壁の内面までの穴 + `gfb_recess` の窪みを抜いたカードケース。平面の隅は R `r` |

## gridfinity_bin_4u.scad

`assets/gridfinity-bin/` の bin 群の共通値 (壁 1.2・床 1.2・ラベル天板 13×1・
境目 R1、カードケースのカード 53.7×85.5・余裕 1・R4) と、サイズ 3 引数の
