...
# - 各 STL が1連結成分であること
# - カードケース: ポケット 54.7×86.5 (R4) が左手前の壁に付いて床の天面から上へ開き、右奥のマスが
#   壁の内面まで抜けて、その床と底が外形に沿った 45° の窪み (皮 1.2、貫通しない) になっていること
set -euo pipefail
...
#    - ポケット: カード 53.7×85.5 + 1 = 54.7 (X) × 86.5 (Y)、隅 R4。左手前の壁に付ける
#      (x -40.55..14.15、y 1.2..87.7)。床は bin の床 (z=5.95)
#    - 穴: 右奥のマス (x 0.25..40.55、y 84..124.3、隅 R4)。埋めは壁の内面まで抜く。
#      床と底のマスは、外形の 3 段に沿って壁厚 1.2・床厚 1.2 を残した相似形の窪みにする:
#      39.1 角 (R4) @ z=5.95 → 45° → 34.8 角 @ 3.8 → 垂直 → 34.8 角 @ 2.0 → 45° → 33.2 角 @ 1.2 (窪みの床)。
#      cut は壁との面の一致を避けるため各段 0.02 小さい (39.08 / 34.78 / 33.18)。底の皮 1.2 で貫通しない
#    - カードの右奥の角 (x ≤13.65、y 84..87.2) が穴の上に張り出す。その下は床の天面から 45° で下がる窪み
# ---------------------------------------------------------------------------
render card "assets/gridfinity-bin/card_case_2x3x4u.scad"
expect_echo card "CONTRACT card units=4 bin=2x3 size=83.5x125.5 h=28 lip=4.4 floor_top=5.95 card=53.7x85.5 pocket=54.7x86.5 at=-40.55,1.2 hole=0.25,84 recess=39.1/34.8/33.2 recess_floor=1.2 r=4"
check_bbox card -41.75 41.75 0 125.5 0 32.4
check_single_solid card
# z=15: 外形と、ポケットと穴が角で繋がった 1 つの空洞。ポケットの縁 (x=14.15、y=87.7) と穴の縁
#    (x=0.25、y=84) の両側で solid/void を測り、隅の R4 を円弧の頂点距離で測る
#    (ポケットの手前左・手前右・奥左、穴の奥左・手前右・奥右。奥右は壁の内側の角 R2.55 の内側に R4 が残る)
check_plan card pocket 15 \
  "loops 2; rect -41.75 41.75 0 125.5;
   void -13.2 44.45; void 13.5 44.45; solid 15 44.45; void -13.2 87.2; solid -13.2 88.2; solid -10 90;
   void 21 104.75; void 7 86.5; solid -0.5 104.75; void 0.75 104.75; solid 21 83.5; void 21 84.5;
   solid 30 40; solid 35 62.75; solid -20 110;
   arc 10.15 5.2 4 10.1 14.2 1.15 5.25; arc -36.55 83.7 4 -40.6 -36.5 83.65 87.75; arc -36.55 5.2 4 -40.6 -36.5 1.15 5.25;
   arc 4.25 120.3 4 0.2 4.3 120.25 124.35; arc 36.55 88 4 36.5 40.6 83.95 88.05; arc 36.55 120.3 4 36.5 40.6 120.25 124.35"
# z=27.5 は埋めの中、z=30 はリップの内側 (埋めは 28 で終わる)
check_plan card fill-top 27.5 "loops 2; solid 30 40; solid -20 110; void 21 104.75; void -13.2 44.45"
check_plan card lip 30 "loops 2; rect -41.75 41.75 0 125.5; loop 0 62.75 81.1 123.1; void 30 40; void 21 104.75; solid 41.2 62.75"
# z=5.5 (床の中): 窪みの上の 45° の途中 (幅 34.78 + 2×1.7 = 38.18)。ポケットの下は床。カードの角の下
#    (x=7): 穴の縁 y=84 から窪みの縁 y=85.66 までは床の棚、その奥は窪み
check_plan card floor 5.5 \
  "loops 2; loop 21 104.75 38.18 38.18; void 21 104.75; solid -13.2 44.45; solid 7 85.0; void 7 86.5;
   solid 21 85.0; void 21 86.0; solid 0.5 104.75; void 2.5 104.75; solid 40.3 104.75"
# z=3 (窪みの垂直部、底のマスは上の面取りの中で 38.0 角): 6 マス + 窪み 34.78 角の 7 loop。
#    右奥のマスの肉 (y 85.75..87.36) は残る
check_plan card recess-mid 3.0 \
  "loops 7; loop 21 104.75 34.78 34.78; loop 21 104.75 38 38; loop -21 104.75 38 38; loop 21 62.75 38 38;
   void 21 104.75; solid 21 86.5; solid 21 62.75; solid -21 104.75"
# z=1.5 (窪みの下の面取りの途中 33.18 + 2×0.3 = 33.78、マスは垂直部 37.2)、z=1.0 (窪みの床 1.2 より下: 皮が残る)
check_plan card recess-lo 1.5 "loops 7; loop 21 104.75 33.78 33.78; loop 21 104.75 37.2 37.2; void 21 104.75"
check_plan card recess-skin 1.0 "loops 6; solid 21 104.75; loop 21 104.75 37.2 37.2"
# Y-Z 断面 (x=21、窪みの中心): 窪みの床 z=1.2、上の 45° (z=5.0 で y=86.16、z=4.0 で y=87.16、±0.25 で挟む)、
#    下の 45° (z=1.6 で y=87.76)、垂直部 (y=87.36)。埋めは y=84 まで、穴は上まで空。窪みは外壁に達しない
check_section_yz card recess-x21 21 \
  "solid 104.75 1.0; void 104.75 1.4; solid 86.5 3; void 88 3; void 86.4 5.0; solid 85.9 5.0; void 87.4 4.0; solid 86.9 4.0;
   void 88.0 1.6; solid 87.5 1.6; solid 44.45 5.5; solid 44.45 6.5; solid 44.45 20; solid 83.5 20; void 84.5 20;
   void 104.75 20; void 104.75 27.5; void 104.75 30; solid 124.9 30; solid 0.6 30; solid 123.8 5.0; solid 122.6 3.0"
# Y-Z 断面 (x=10、カードの右奥の角の下): ポケットの床の上は空、角の張り出し (y 84.25..87.2) の下に
#    45° で下がる窪みがある (張り出しは y 84..87.2)。穴とポケットは繋がって上まで空
check_section_yz card corner-x10 10 \
  "solid 44.45 5.5; void 44.45 6.5; void 44.45 20; void 86.5 6.5; void 86.5 20; void 100 20; solid 124.9 20;
...
check_section card hole-y 104.75 \
  "solid 21 1.0; void 21 1.4; solid 3.0 3.0; void 4.0 3.0; void 2.7 5.0; solid 2.1 5.0; void 3.7 4.0; solid 3.1 4.0;
