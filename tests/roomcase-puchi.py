#!/usr/bin/env python3
"""天馬用の前後STLを実測: 左端数セル、42mm格子、床なし、奥R4。"""
...
with tempfile.TemporaryDirectory(prefix="roomcase-puchi-test-") as temp:
    ...
    for name, rows, length, origin in [("front", 4, 168, 0), ("rear", 3, 153, 168)]:
        ...
        for z, span, radius in [(0.35, 36.7, 1.6), (1.6, 37.4, 1.95), (4, 40.4, 3.45), (4.64, 41.7, 4.1)]:
            ...
            if spacer:
                for sign in [-1, 1]:
                    arc = [p for p in outer if sign * p[0] > 111.55 and p[1] > 149.05]
                    assert len(arc) >= 5, "rear corner is not rounded"
                    assert all(abs(math.dist(p, (sign * 111.5, 149)) - 4) < 0.03 for p in arc), "rear corner is not R4"
    ...
    print("roomcase-puchi: split pitch 42mm, rear R4 and rear spacer passed")
