...
with tempfile.TemporaryDirectory(prefix="roomcase-puchi-test-") as temp:
    ...
    for name, rows, length, origin in [("front", 4, 168, 0), ("rear", 3, 153, 168)]:
        ...
        near(bounds(closed_mesh(stl)), (-115, 115, 0, length, 0, 4.65))
        # 全ソケットを各段で測る。X幅だけが22mm短く、角R・Y幅・深さは同じ。
        for z, span, radius in [(0.35, 36.7, 1.6), (1.6, 37.4, 1.95), (4, 40.4, 3.45), (4.64, 41.7, 4.1)]:
            ...
            outer = loop_at(loops, (-115, 115, 0, length if spacer else rows * 42))
            assert len(loops) == 1 + 6 * rows + (24 if spacer else 0), (name, z, len(loops))
            for row in range(rows):
                cy = 21 + row * 42
                for cx, width in [(-105, span - 22), *[(x, span) for x in [-74, -32, 10, 52, 94]]]:
            ...
            if spacer:
                for sign in [-1, 1]:
                    arc = [p for p in outer if sign * p[0] > 110.05 and p[1] > 148.05]
                    assert len(arc) >= 5, "rear corner is not rounded"
                    assert all(abs(math.dist(p, (sign * 110, 148)) - 5) < 0.03 for p in arc), "rear corner is not R5"
                # 奥の27mmはソケットを持たない位置決め縁。X筋交いの交点と外枠。
                for cx in [-105, -74, -32, 10, 52, 94]:
                    assert inside(loops, (cx, 139.5)) and inside(loops, (cx, 152))
            for sign in [-1, 1]:
                assert inside(loops, (sign * 114.95, 0.05)), "front corners must remain square"
        for row in range(rows):
            cy = 21 + row * 42
            vertical = section(stl, work, "y", cy)
            for cx, half_width in [(-105, 6.9), *[(x, 17.9) for x in [-74, -32, 10, 52, 94]]]:
                empty_rect(vertical, (cx - half_width, cx + half_width, -0.1, 4.8))
            for x in [-114.95, -95, -53, -11, 31, 73, 114.95]:
                assert inside(vertical, (x, 4.6)), ("socket wall missing", name, row, x)
            assembled_rows.append(cy + origin)
        if name == "rear":
            near(bounds([p for loop in section(stl, work, "y", 139.5) for p in loop]), (-115, 115, 0, 2))
