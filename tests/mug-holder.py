...
with tempfile.TemporaryDirectory(prefix="mug-holder-test-") as temp:
    ...
    near(bounds(vertices), (-44.3, 44.3, -44.3, 53.7, 0, 35.6))

    # 収納円筒は内径84.6。底Z3からR2で繋ぎ、上の1mmだけR1で広げる。
    for z, radius in [(3.1, 40.3 + math.sqrt(4 - 1.9 ** 2)),
                      (4, 40.3 + math.sqrt(3)), (5.01, 42.3), (34.59, 42.3),
                      (35.1, 43.3 - math.sqrt(0.75)), (35.59, 43.3 - math.sqrt(1 - 0.99 ** 2))]:
    ...
    empty_rect(vertical, (-40.28, 40.28, 3.01, 35.7))
    empty_rect(vertical, (-42.28, 42.28, 5.01, 35.7))
    for sign in [-1, 1]:
        for z in [5.1, 15, 34.5]:
            assert inside(vertical, (sign * 42.33, z))
            assert inside(vertical, (sign * 44.27, z))
            assert not inside(vertical, (sign * 44.33, z))
        for cx, cz, radius, z0, z1 in [(40.3, 5, 2, 3.001, 4.999), (43.3, 34.6, 1, 34.601, 35.599)]:
            arc = [(sign * x, z) for loop in vertical for x, z in loop if sign * x > 40.3 and abs(sign * x - cx) <= radius + 0.02 and z0 < z < z1]
    ...
    for z in [0.01, 1.5, 2.99]:
        ...
        holes = [loop for loop in floor if max(math.hypot(x, y) for x, y in loop) < 42.3]
        ...
        for hole in holes:
            ...
            for x, y in hole:
                ...
                assert math.hypot(x, y) <= 39.31, "floor perimeter is too thin"
        ...
        assert inside(floor, (40.5, 0)), "solid floor perimeter is missing"
    empty_rect(vertical, (-3.44, 3.44, -0.1, 3.01))
    assert inside(vertical, (39.5, 2.99)) and not inside(vertical, (39.5, 3.01))

    # 頭は対辺14.2 (=13.8+0.4)、軸は対辺8.4の上下flat六角。穴中心Z25.35。
    for y, flat in [(42.5, 14.2), (45.49, 14.2), (45.51, 8.4), (49.89, 8.4)]:
    ...
    assert inside(section(stl, work, "y", 45.51), (5, 25.35)), "head bearing shoulder is missing"

    # 上下の処理範囲を除き、回転止めは幅2、逃げは24.4x3.8を維持する。
    for z in [11.5, 16, 33, 34.59]:
        plan = section(stl, work, "z", z)
        empty_rect(plan, (-12.2, 12.2, 49.9, 53.8))
        assert inside(plan, (0, 49.88)), "clamp bearing face is missing"
        for sign in [-1, 1]:
            assert inside(plan, (sign * 12.22, 51.5))
            assert inside(plan, (sign * 14.18, 51.5))
            assert inside(plan, (sign * 13.2, 53.68)), "rotation stop no longer reaches the desk"
            assert not inside(plan, (sign * 14.22, 51.5)), "rotation stop width is wrong"
    back = section(stl, work, "y", 51.5)
    for sign in [-1, 1]:
    ...
    for z, inset in [(0.1, 0.3), (0.3, 0.1), (0.41, 0),
        ...
        plan = section(stl, work, "z", z)
        near(bounds([p for loop in plan for p in loop]), (-44.3 + inset, 44.3 - inset, -44.3 + inset, 53.7 - inset))
        empty_rect(plan, (-12.2, 12.2, 49.9 - inset, 53.8))
        for sign in [-1, 1]:
            for x in [12.2 + inset + 0.035, 14.2 - inset - 0.035]:
                assert inside(plan, (sign * x, 51.5)), "rounded/chamfered stop is too thin"
            assert not inside(plan, (sign * (12.2 + inset - 0.035), 51.5)), "stop inner edge is sharp"
            assert not inside(plan, (sign * (14.2 - inset + 0.035), 51.5)), "stop outer edge is sharp"
        assert inside(plan, (0, 49.9 - inset - 0.035)), "mount upper/bottom edge lost too much material"

    print("mug-holder: 84.6mm bore, 2mm wall, closed solid, flat-up 8.4/14.2 hex holes, 3mm floor, R2 inner floor, R1 top, C0.4 bottom and clamp fit passed")
