...
CONTRACT="CONTRACT plate=240.6x313.5 grid=5.5x7 pitch=42 socket=41.5/37.2/35.8 fit=0.1 depth=4.65 floor=0 rim=L4.8/R4.8/F1.5/B18 xwin=2 corner=3"
...
# ---------------------------------------------------------------------------
# 6. 奥片の奥側 2 隅は実測チップで確定した R3。右の中心は (117.3, 141)。
#    (119.8, 143.5) は中心から約3.54なので void、(119.3, 143) は約2.83で solid。
#    直線部を除く左右の円弧頂点が (±117.3, 141) から距離3にあることを測る。
#    前片の 4 隅と奥片の前側 2 隅は直角のまま
# ---------------------------------------------------------------------------
check_plan rear corners 2.0 \
  "void 119.8 143.5; void -119.8 143.5; solid 119.3 143; solid -119.3 143; solid 117.3 143.5;
   arc 117.3 141 3 117.35 121 141.05 145; arc -117.3 141 3 -121 -117.35 141.05 145;
