...
#   grid <x0> <dx> <nx> <y0> <dy> <ny> <span> — 外形以外の全 loop の中心の集合が
#       格子 {x0+i*dx} × {y0+j*dy} と完全一致し、各 loop が span 角である
...
for cond in spec.split(";"):
    ...
    if kind == "loops":
        ...
        holes = [bbox(l) for l in loops if l is not body]
        got = {(round((b[0] + b[1]) / 2, 3), round((b[2] + b[3]) / 2, 3)) for b in holes}
        bad_span = [b for b in holes
                    if abs((b[1] - b[0]) - span) > 0.05 or abs((b[3] - b[2]) - span) > 0.05]
        ...
        print(f"plan {name}: grid {len(matched)}/{len(want)} centers matched,"
              f" {len(extra)} extra, {len(bad_span)} off-span (want {span})")
        if len(matched) != len(want) or extra or bad_span:
            print(f"plan {name}: missing {sorted(want - matched)} extra {extra} off-span {bad_span[:3]}")
            ok = False
...
# ---------------------------------------------------------------------------
CONTRACT="CONTRACT plate=238x316 grid=5x7 pitch=42 socket=41.5/37.2/35.8 depth=4.65 floor=1 rim=L14/R14/F4/B18"
expect_echo front "$CONTRACT split=front rows=4 len=172"
expect_echo rear "$CONTRACT split=rear rows=3 len=144"

# 外形。X は中央が 0、Y は各片の前端が 0、Z=0 が底面。高さは床 1 + ソケット 4.65
check_bbox front -119 119 0 172 0 5.65
check_bbox rear -119 119 0 144 0 5.65
check_single_solid front
check_single_solid rear

# ---------------------------------------------------------------------------
# 1. ソケットの配置 (42mm ピッチ)。grid 節で 20 / 15 個すべての中心を集合として固定する。上の面取り (Z 3.5..5.65) の中の z=5.0 で
#    切ると、各ソケットは 41.5 − 2×0.65 = 40.2 角の loop になる。
...
# ---------------------------------------------------------------------------
check_plan front cells 5.0 \
  "loops 21; rect -119 119 0 172; grid -84 42 5 25 42 4 40.2;
...
check_plan rear cells 5.0 \
  "loops 16; rect -119 119 0 144; grid -84 42 5 21 42 3 40.2;
...
# ---------------------------------------------------------------------------
# 2. ソケット輪郭の3段 (床 1 の上に、0.7 面取り / 1.8 垂直 / 2.15 面取り)。
#    垂直部 (Z 1.7..3.5) の z=2.6 で切ると 37.2 角、下の面取り (Z 1.0..1.7) の
#    z=1.35 で切ると 35.8 + 2×0.35 = 36.5 角になる
# ---------------------------------------------------------------------------
check_plan front wall 2.6 "loops 21; loop -84 25 37.2 37.2; loop 0 109 37.2 37.2"
check_plan front chamfer-bottom 1.35 "loops 21; loop -84 25 36.5 36.5; loop 84 151 36.5 36.5"
check_plan rear wall 2.6 "loops 16; loop 84 105 37.2 37.2"

# ---------------------------------------------------------------------------
# 3. 縦断面 (X-Z) で、ソケットが床の上 (z=1) から天面まで抜けていること、
#    床がソケットの下で連続していること、縁 (|x| > 105) が全高で詰まっていること。
#    ソケットの穴は z=1.0 で 35.8 角 (半幅 17.9) なので、半幅 17.5 の矩形
#    x∈[-101.5,-66.5] z∈[1.05,5.6] は完全に空でなければならない
# ---------------------------------------------------------------------------
check_section front row1 25 \
  "void_rect -101.5 -66.5 1.05 5.6; void_rect -17.5 17.5 1.05 5.6;
   solid -84 0.5; solid 0 0.5; solid -63 2.5; solid 63 2.5;
   solid -112 3; solid 112 3; solid -118.5 5.5; void -84 3"
check_section rear row3 105 \
  "void_rect 66.5 101.5 1.05 5.6; solid 84 0.5; solid 105 3; solid -112 3; void 84 3"
# 縁の内側 (前片の y=2, 奥片の y=135) は全幅で詰まった板である
check_section front front-rim 2 "solid 0 3; solid -118.5 3; solid 118.5 3; void 0 5.7"
check_section rear rear-rim 135 "solid 0 3; solid -118.5 3; solid 118.5 3; void 0 5.7"
