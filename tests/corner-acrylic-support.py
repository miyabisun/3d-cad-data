...
SOURCE = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else ROOT / "assets/steel-rack/500x400/corner_acrylic_support.scad"
...
with tempfile.TemporaryDirectory(prefix="corner-acrylic-support-") as temp:
    ...
    near(bounds(closed_mesh(stl)), (0, 27, 0, 27, 0, 25.9))
    ...
    for shift in [0, 0.4 / math.sqrt(3)]:
        a = f'translate([7.1,{15.7-shift},15.2]) hex_x_flat_up(10,5);'
        b = f'translate([{15.7-shift},7.1,15.2]) hex_y_flat_up(10,5);'
    ...
    no_collision(f'intersection() {{ import("{stl}"); hull() {{ {m4} translate([0,0,30]) {{ {m4} }} }} }}')
    ...
    shanks = '''
      ...
      translate([17,17,-5]) cylinder(d=4,h=16,$fn=48);
    '''
    no_collision(f'intersection() {{ import("{stl}"); union() {{ {shanks} }} }}')
    print("corner-acrylic-support: dimensions, both M6 seats/entries, M4 pocket, R2/R5/R10 and nut clearance passed")
