...
# check_z_levels <name> <z:max_abs_x> ... — STL の水平面を実測する。
# この部品は垂直面と水平な上下面しか持たないので、頂点の distinct Z が
# そのまま「水平面の一覧」になる。各 Z にはその高さの頂点の max|X| を添える。
# max|X| が在ると、その平面が外形の段なのか穴の段なのかを区別できる:
# 外形の段なら外周 (18) まで届き、穴の段なら穴の縁 (六角の対角7.737) で止まる
check_z_levels() {
  name=$1
  shift
  python3 - "$WORK/$name.stl" "$name" "$@" <<'PYEOF' || fail=1
...
for x, y, z in vs:
    k = round(z, 4)
    levels[k] = max(levels.get(k, 0.0), abs(x))
got = sorted(levels.items())
print(f"levels {name}: " + " ".join(f"{z:.3f}:{x:.3f}" for z, x in got))
ok = len(got) == len(want) and all(
    abs(z - wz) < 0.05 and abs(x - wx) < 0.05
    for (z, x), (wz, wx) in zip(got, want))
if not ok:
    print(f"levels {name}: want " + " ".join(f"{z}:{x}" for z, x in want))
print(f"levels {name}: {'ok' if ok else 'FAIL'}")
sys.exit(0 if ok else 1)
PYEOF
}
...
# ---------------------------------------------------------------------------
# 1. レバー = 楕円ローブ2枚 ∪ 中央ハブ円へ closing (R2) を掛けた平面輪郭を、
#    Z 0..10 の全高へ一様に押し出したもの。全長 (X) はちょうど 36。
#    ハブ円 (Φ21.473 = 六角対角15.473 + 壁3×2) はローブの短径 8 より太いので、
#    外形の Y 幅はハブが決める。
#    z=2.5 は下段の穴 (M8 軸の通し穴 Φ8.4) が開く高さである
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. 上半分 (Z 5..10) にもレバーが在ること。ローブの高さは部品の全高 10 で、
#    中央ハブ内にボルト先端の六角 (二面幅13.4) を受けるポケットが上へ開く。
#    旧構成 (上段は中央ハブだけのボス・ローブは Z=5 で終わる) は superseded
# ---------------------------------------------------------------------------
# z=7.5: レバーの外形1本 + 六角ポケット1本 = 2 loop。外形は下段と同じ全長36 で、
# ローブの短径 8 も接合の R2 も z=2.5 と同一断面になる。六角は $fn=6 で対角が
# X (15.473)、二面幅が Y (13.4) に出る。ローブが Z=5 で終わる旧形状なら
# rect の X 幅が 21.473 に縮み、x=±17 が void になって red
check_plan knob lever-top 7.5 \
  "loops 2;
   rect -18 18 -10.736 10.736;
   loop 0 0 15.473 13.4;
   void 0 0; solid 0 8; solid 9 0; void 0 11.5;
   solid 17 0; solid -17 0;
   solid 12 3; void 12 3.8; solid -12 3; void -12 3.8;
   solid 10.2 3.8; solid -10.2 -3.8"

# ---------------------------------------------------------------------------
# 3. 穴が段を切り替える高さ (Z=5) と、穴が Z 0..10 で連続して上へ抜けること。
#    外形はもう段を持たないので、Z=5 で切り替わるのは穴だけである。
#    水平断面は「その高さに何が在るか」しか見えないので、境界を Z=4 へ
#    ずらした形状や、Z=5 付近で穴を塞ぐ薄い天井を素通しにしてしまう。
#    ここは境界を跨ぐ2枚の水平断面と、軸を通る縦断面で測る
# ---------------------------------------------------------------------------
# 境界の直下 (z=4.9): 穴はまだ Φ8.4 の丸。外形は全長36。
# 境界が Z=4 へ下がっていれば、ここに六角が出て red
check_plan knob step-below 4.9 \
  "loops 2; rect -18 18 -10.736 10.736; loop 0 0 8.4 8.4; solid 6 0"
# 境界の直上 (z=5.1): 穴はもう六角。外形は全長36 のまま変わらない。
# 境界が Z=6 へ上がっていれば、ここに丸穴が出て red
check_plan knob step-above 5.1 \
  "loops 2; rect -18 18 -10.736 10.736; loop 0 0 15.473 13.4; void 6 0"
...
#   ③ ローブが全高で立っていること: 腕の上端付近 x=±16, z=9.8 が材料になる。
#      ローブが Z=5 で終わる旧形状ならここは void で red
check_section knob axis 0 \
  "loop -11.1 5 13.8 10; loop 11.1 5 13.8 10;
   ...
   solid -16 0.2; solid 16 0.2; solid -16 9.8; solid 16 9.8"

# ---------------------------------------------------------------------------
# 4. 形状健全性: 外形 bbox・水平面の一覧・単一連結成分。
#    Z は全高 10、X は全長 36、Y はハブ径 21.473
# ---------------------------------------------------------------------------
check_bbox knob -18 18 -10.736 10.736 0 10
# 水平面は3枚しかない。底面 z=0 と天面 z=10 はどちらも外周 (18) まで届く
# 一様押し出しの端で、間の z=5 は穴が Φ8.4 から六角へ太る段だけである
# (max|X| が六角の対角半分 7.737 で止まる = 外形には段が無い)。
# 上段ボスの旧構成なら z=5 の max|X| が 18 (レバーの上面)、z=10 が 10.736
# (ボスの天面) になって red
check_z_levels knob 0:17.997 5:7.737 10:17.997
...
# レバーは全高 10mm の一本押し出し (user 指定「ローブの高さ 10mm」)
expect_echo knob 'CONTRACT hand_knob: lever_h = 10'
# 穴だけが2段。下段 (通し穴) 5mm + 上段 (六角ポケット) 5mm
expect_echo knob 'CONTRACT hand_knob: hole_heights = [5, 5]'
